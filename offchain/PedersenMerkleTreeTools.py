...
class PedersenMerkleTree(object):
    def __init__(self):
        self.reset_tree()

    def hash_function(self, x):
        return PedersenHasher('test').hash_bytes(x).compress()
    ...
    def add_leaf(self, values, do_hash=False):
        ...
        for v in values:
            if do_hash:
                v = v.encode('utf-8')
                v = self.hash_function(bytes.fromhex(v)).hex()
            v = bytes.fromhex(v)
        ...
        if len(proof) == 0:
            ...
            for p in proof:
                try:
                    ...
                    sibling = bytes.fromhex(p['left'])
                    proof_hash = self.hash_function(sibling + proof_hash)
                except:
                    # the sibling is a right node
                    sibling = bytes.fromhex(p['right'])
